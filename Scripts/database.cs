...
namespace DBDiplomZernoKolhoz.Scripts
{
    class database
    {
        ...
        public string selectJurnal = "SELECT Журнал.КодЖурнала, Журнал.Дата, Зернопродукция.Культура, Склад.Наименование, Журнал.Брутто, Журнал.Тара, Журнал.Нетто, (Водитель.Фамилия & ' ' & Водитель.Имя & ' ' & Водитель.Отчество) AS Водитель, (Комбайнер.Фамилия & ' ' & Комбайнер.Имя & ' ' & Комбайнер.Отчество) AS Комбайнер, Поле.Наименование FROM Склад INNER JOIN(Поле INNER JOIN (Комбайнер INNER JOIN (Зернопродукция INNER JOIN (Водитель INNER JOIN Журнал ON Водитель.КодВодителя = Журнал.КодВодителя) ON Зернопродукция.КодЗернопродукции = Журнал.КодЗернопродукции) ON Комбайнер.КодКомбайнера = Журнал.КодКомбайнера) ON Поле.КодПоля = Журнал.КодПоля) ON Склад.КодСклада = Журнал.КодСклада ";
